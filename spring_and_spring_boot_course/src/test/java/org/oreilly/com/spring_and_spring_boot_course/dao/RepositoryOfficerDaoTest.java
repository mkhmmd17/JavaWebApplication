...
@SpringBootTest
@Transactional
class RepositoryOfficerDaoTest {
    ...
    void findAllByLastNameContainingAndRank() {
        List<Officer> officers = dao.findAllByLastNameContainingAndRank("Y", Rank.CAPTAIN);
        assertThat(officers).hasSize(2);
        assertThat(officers.stream().map(Officer::getLastName))
                .containsExactlyInAnyOrder("WYNNE", "YAN");
    }
